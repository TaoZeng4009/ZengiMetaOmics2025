...
#### Enrichment analysis
dir.create("Results/04.Enrichment")
...
p <-ggplot(enrichment,aes(Rich_factor,layer3)) +
    ...
    labs(title="Statistics of pathway enrichment for DAMs in intestinal tract",
         size = "DAMs") +
    ...
    theme(panel.background = element_rect(fill = "white",colour='black'),
          ...
          strip.text.y = element_text(angle = 0,colour = "black",size = 10)) +
    theme(plot.title = element_text(size=16,colour = "black",hjust = 1))

pdf("Results/04.Enrichment/enrichment.pdf",width = 8,height = 5)
p
dev.off()

### DAMs
dir.create("Results/01.Venn")
...
library(VennDiagram)
venn.diagram(up,filename = "Results/01.Venn/up.png",
...
venn.diagram(down,filename = "Results/01.Venn/down.png",
             height = 5400,width = 5400,
             resolution = 600,imagetype = "png",units = "px",
             lwd = 2,lty = 1,fill = cbbPalette,cex = 1.5,
             cat.cex = 2,alpha = 0.8,margin = 0.05,fontface = 2,
...
DAM1 <- read.table("Input/metabolome1.txt",header = TRUE,sep = "\t") 
DAM2 <- read.table("Input/metabolome2.txt",header = TRUE,sep = "\t",quote = "")
DAM <- merge(DAM1,DAM2)
aa <- DAM$ID[DAM$ID %in% aa]
bb <- DAM$ID[DAM$ID %in% bb]
rownames(DAM) <- DAM$Name
DAM <- DAM[,-10]
DAM <- DAM[DAM$ID %in% c(aa,bb),]
DAM <- DAM[,-5]
...
library(linkET)
network_data <- cbind(t(otu[,-ncol(otu)]),t(DAM))
res <- correlate(t(otu[,-ncol(otu)]),t(DAM),method = "spearman",engine = "WGCNA")
...
nodes$Type <- ifelse(nodes$name %in% aa,"Up-DAM",
                     ifelse(nodes$name %in% bb,"Down-DAM","Gut bacteria"))
...
pdf("Results/02.Network/Network_label.pdf",width = 10,height = 10)
p3
dev.off()
...
pdf("Results/02.Network/Network_nolabel.pdf",width = 10,height = 10)
p4
dev.off()
...
ARG_pcoa2<-ggplot(plotdata, aes(PC1, PC2)) +
    ...
    geom_text(aes(0,-0.55),label = paste("Adonis test: R2 = ",round(ARG.adonis$R2[1],3),
                                            ", p-value = ",round(ARG.adonis$`Pr(>F)`[1],3),sep = ""),
              size = 5) + 
    scale_fill_manual(values=cbbPalette)+
    labs(title="PCoA - Intestinal metabolites") + 
...
pdf(file = "Results/03.PCoA/PCoA_ellipse.pdf",width = 5.4,height = 4.5)
ARG_pcoa2
dev.off()

## Correlation
dir.create("Results/05.Correlation")
aa <- c("OTU191","OTU125","OTU100","OTU53","OTU60","OTU1","OTU16","OTU177","OTU98",
        "OTU37","OTU203","OTU155","OTU73","OTU17","OTU65","OTU39")
otu <- otu[aa,]
otu$taxonomy <- gsub(" .*","",otu$taxonomy)
rownames(otu) <- gsub("OTU","ASV",rownames(otu))
rownames(otu) <- paste(rownames(otu),"(",otu$taxonomy,")",sep = "")
bb <- c("Com_31_neg","Com_73_pos","Com_78_neg","Com_54_pos","Com_53_pos")
DAM <- merge(DAM1,DAM2)
rownames(DAM) <- DAM$Name
DAM <- DAM[,-10]
DAM <- DAM[DAM$ID %in% bb,]
DAM <- DAM[,-5]

otu <- otu[,-ncol(otu)]
library(pheatmap)
library(psych)
res <- corr.test(t(otu),t(DAM),method = "spearman",adjust = "holm")
pdf("Results/05.Correlation/heatmap.pdf",width = 10,height = 11)
pheatmap(res$r, fontsize_number=35,fontsize = 20,number_color = "white",
