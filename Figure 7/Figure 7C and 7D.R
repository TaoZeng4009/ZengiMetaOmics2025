...
Gene <- read.table("Input_C_D/genes.fpkm.txt",header = TRUE,sep = "\t",row.names = 1)
group <- read.table("Input_C_D/group.txt",header = TRUE,sep = "\t")
...
ARG_pcoa2<-ggplot(plotdata, aes(PC1, PC2)) +
    ...
    geom_text(aes(0,-0.2),label = paste("Adonis test: R2 = ",round(ARG.adonis$R2[1],3),
                                         ", p-value = ",round(ARG.adonis$`Pr(>F)`[1],3),sep = ""),
              size = 5) + 
    scale_fill_manual(values=cbbPalette)+
    labs(title="PCoA - Liver transcriptome") + 
...
enrichment <- read.table("Input_C_D/enrichment.txt",header = TRUE,sep = "\t")
...
p <-ggplot(enrichment,aes(Rich_factor,layer3)) +
    ...
    labs(title="Statistics of pathway enrichment for DEGs in liver",
         size = "DEGs") +
    ...
    theme(panel.background = element_rect(fill = "white",colour='black'),
          ...
          legend.key=element_blank(),legend.position = c(0.85,0.2),
          legend.background = element_rect(colour = "black"),
          legend.key.height=unit(0.4,"cm"),
          strip.text.y = element_text(angle = 0,colour = "black",size = 12)) +
    theme(plot.title = element_text(size=20,colour = "black",hjust = 1))

pdf("Results/03.Enrichment/enrichment.pdf",width = 8,height = 7)
p
dev.off()

### DAMs
dir.create("Results/04.Venn")
up <- read.table("Input_C_D/up.txt",header = TRUE,sep = "\t")
...
down <- read.table("Input_C_D/down.txt",header = TRUE,sep = "\t")
...
group <- read.table(file = "Input_C_D/group.txt",header = TRUE,sep = "\t")
otu <- read.table("Input_C_D/taxa_table.xls",header = FALSE,sep = "\t",quote = "",
                  row.names = 1)
colnames(otu) <- otu[1,]
otu <- otu[-1,]
