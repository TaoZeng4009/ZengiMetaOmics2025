...
Gene <- read.table("Input_A_B/genes.fpkm.txt",header = TRUE,sep = "\t",row.names = 1)
group <- read.table("Input_A_B/group.txt",header = TRUE,sep = "\t")
...
ARG_pcoa2<-ggplot(plotdata, aes(PC1, PC2)) +
    ...
    geom_text(aes(0.1,-0.3),label = paste("Adonis test: R2 = ",round(ARG.adonis$R2[1],3),
                                         ", p-value = ",round(ARG.adonis$`Pr(>F)`[1],3),sep = ""),
              size = 5) + 
    scale_fill_manual(values=cbbPalette)+
    labs(title="PCoA - Heart transcriptome") + 
...
enrichment <- read.table("Input_A_B/enrichment.txt",header = TRUE,sep = "\t")
...
p <-ggplot(enrichment,aes(Rich_factor,layer3)) +
    ...
    labs(title="Statistics of pathway enrichment for DEGs in heart",
         size = "DEGs") +
...
pdf("Results/03.Enrichment/enrichment.pdf",width = 7.5,height = 6.5)
p
dev.off()

### DAMs
dir.create("Results/04.Venn")
up <- read.table("Input_A_B/up.txt",header = TRUE,sep = "\t")
...
down <- read.table("Input_A_B/down.txt",header = TRUE,sep = "\t")
...
group <- read.table(file = "Input_A_B/group.txt",header = TRUE,sep = "\t")
otu <- read.table("Input_A_B/taxa_table.xls",header = FALSE,sep = "\t",quote = "",
                  row.names = 1)
colnames(otu) <- otu[1,]
otu <- otu[-1,]
...
res <- correlate(t(otu[,-ncol(otu)]),t(Gene),method = "spearman",engine = "WGCNA")
library(reshape2)
res1 <- res
data.r <- melt(res$r)
data.p <- melt(res$p)
...
aa <- result %>% group_by(Source) %>% summarise(value = length(Source))
aa <- aa[order(aa$value,decreasing = TRUE),]
write.table(aa,"Results/05.Network/degree_asv.txt",sep = "\t",row.names = FALSE)

bb <- result %>% group_by(Target) %>% summarise(value = length(Target))
bb <- bb[order(bb$value,decreasing = TRUE),]
write.table(bb,"Results/05.Network/degree_gene.txt",sep = "\t",row.names = FALSE)
